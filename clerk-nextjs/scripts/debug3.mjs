...
function extrairHistoricoConsumo(texto) {
  ...
  function coletarNums(startLine, maxLines) {
    const acc = []
  ...
  for (const { re, tipo } of grandezas) {
    for (let i = 0; i < linhas.length; i++) {
      ...
      for (let j = 0; j < N; j++) {
        ...
        if (tipo === 'consumoFP') entrada.consumoForaPontaKwh = historicos[j]
        else if (tipo === 'consumoP') entrada.consumoPontaKwh = historicos[j]
        else if (tipo === 'demandaFP') entrada.demandaForaPontaKw = historicos[j]
        else if (tipo === 'demandaP') entrada.demandaPontaKw = historicos[j]
      }
      break
    }
  }
...
async function main() {
  ...
  if (!f) { console.log('Fatura 02/2025 nao encontrada'); await prisma.$disconnect(); return }
  ...
  console.log('\n=== JANELA ===')
  janela.split('\n').forEach((l, i) => console.log('  L' + i + ': ' + JSON.stringify(l)))
